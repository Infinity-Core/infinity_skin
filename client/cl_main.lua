...
-- [[ LOAD SKIN CREATOR ]]
----
RegisterNetEvent('infinity_skin:loadSkinTrigger')
AddEventHandler('infinity_skin:loadSkinTrigger', function()
    TriggerServerEvent("infinity_skin:loadSkin")
end)

RegisterNetEvent('infinity_skin:fixwoman')
AddEventHandler('infinity_skin:fixwoman', function()
    if not IsPedMale(PlayerPedId()) then
       Citizen.InvokeNative(0xD3A7B003ED343FD9, PlayerPedId(), tonumber(26925726), false,true,true)
       Citizen.InvokeNative(0xD3A7B003ED343FD9, PlayerPedId(), tonumber(37736976), false,true,true)
       Citizen.InvokeNative(0xD3A7B003ED343FD9, PlayerPedId(), tonumber(366647875), false,true,true)
       Citizen.InvokeNative(0xD3A7B003ED343FD9, PlayerPedId(), tonumber(4745637), false,true,true)
       Citizen.InvokeNative(0x1902C4CFCC5BE57C, PlayerPedId(), 0x29932164) 
...
RegisterNetEvent('infinity_skin:MakeMySkin')
AddEventHandler('infinity_skin:MakeMySkin', function(EntityNew)
    local PlayerPed         = PlayerPedId()
    local PlayerPedCoords   = GetEntityCoords(PlayerPed)
    StartPlayerTeleport(PlayerPed, PlayerPedCoords.x, PlayerPedCoords.y, PlayerPedCoords.z -1, 200.0, false, true, true)
    SetEntityHeading(PlayerPed, 274.05)
    FreezeEntityPosition(PlayerPed, true)
    Wait(255)
    ShowSkinCreator(true, EntityNew)
...
function ShowSkinCreator(enable, EntityNew)
    if EntityNew == 1 then 
        FreezeEntityPosition(PlayerPedId(), true)
        TriggerServerEvent('infinity_skin:instance', true)
    else
...
----
-- [[ USER SKIN CMD  - RELOAD SKIN/CLOTHES ]]
----
TriggerEvent('chat:addSuggestion', '/loadskin', 'Load your lastest skin', {})
RegisterCommand("loadskin", function(source, args, rawCommand)
    source      = GetPlayerServerId(PlayerId())
    TriggerServerEvent("infinity_skin:LoadMySkin", source)
end)

----
-- [[ USER SKIN CMD  - RELOAD SKIN/CLOTHES ]]
----
TriggerEvent('chat:addSuggestion', '/skin', 'Make a new skin', {})
RegisterCommand("skin", function(source, args, rawCommand)
    TriggerEvent("infinity_skin:MakeMySkin", 0)
    source      = GetPlayerServerId(PlayerId())
    exports.infinity_core:notification(source, 'Skin Creator',' Make your unique Skin', 'center_right', 'redm_min', 3500)
end)
...
RegisterNUICallback('saveSkin', function(data, cb)
    TriggerServerEvent('infinity_skin:instance', false)
    ...
    -- Make a new Skin for New Character
    if data.NewCaracter == 1 then 
        TriggerServerEvent("infinity_skin:NewSkinCreateFirstJoin", data)
    end
    -- Make a New Skin for a current Character
    if data.NewCaracter == 0 then 
        TriggerServerEvent("infinity_skin:createSkin", data)
    end
    -- RESURECT THE PLAYER FOR SET IN THE WORLD (no ressurect = no play)
    local ply = PlayerPedId()
    ResurrectPed(PlayerPedId(ply))
    Citizen.InvokeNative(0x50C803A4CD5932C5, true)
    Gethealth                   = GetEntityHealth(PlayerPedId())
    SetEntityHealth(ply, Gethealth, 0)
    Citizen.InvokeNative(0xC6258F41D86676E0, PlayerPedId(), 0, 100)
    Citizen.InvokeNative(0xC6258F41D86676E0, PlayerPedId(), 1, 100)
    -- RESURECT THE PLAYER FOR SET IN THE WORLD
    Wait(1500)
    DoScreenFadeIn(1500)
end)

RegisterNetEvent('infinity_skin:ApplySkinPlayer')
AddEventHandler('infinity_skin:ApplySkinPlayer', function(_data, target , clothes)
    Citizen.CreateThread(function()
...
RegisterNUICallback('updateBody', function(data, cb)
    TriggerEvent("infinity_skin:updateBody" , data)
end)

RegisterNetEvent('infinity_skin:updateBody')
AddEventHandler('infinity_skin:updateBody', function(data)
